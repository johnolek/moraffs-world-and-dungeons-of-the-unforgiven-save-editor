<script lang="ts">
  ...
  import { helpCycleColour } from './help-colours';
  import { parseDoc, searchDoc, type Inline, type LinkTarget } from './markdown';
  import PixelText from './PixelText.svelte';

  /**
   * One of the site's own documents, drawn from its Markdown with a table of contents beside it.
   *
   * `searchLabel` is what the box above the contents says, and `nothingFound` what stands in for
   * the document when nothing in it matches the box; both are in the words of the tab the
   * document belongs to.
   */
  let {
    source,
    searchLabel,
    nothingFound,
  }: { source: string; searchLabel: string; nothingFound: string } = $props();

  const all = $derived(parseDoc(source));
  ...
  const sections = $derived(searchDoc(all, search));
...
<div class="page">
  <div class="index">
    <input type="search" placeholder={searchLabel} bind:value={search} />
    <nav class="scroll">
