<script lang="ts">
  import './editor.css';
  import { app, currentEntry, watchingCharacterOn } from '../app-state.svelte';
  import {
    characterEdited,
    importCharacter,
  ...
  //
  // Only while the editor is the tab on screen (`watchingCharacterOn`). A game writes the
  // character's record back to the roster after every key, which is a fresh array of bytes each
  // time, and opening the whole document again for each of them is most of what made walking
  // stutter.
  $effect(() => {
    if (!watchingCharacterOn('editor')) return;
    const current = currentEntry();
    if (!current) {
      doc = null;
      return;
