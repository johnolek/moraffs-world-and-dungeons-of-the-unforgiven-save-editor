...
- The two `PORTED-FROM.md` pages are generated by `pnpm build:ported` from the citations
  the port already carries; never edit them by hand. `scripts/ported-from.mts` does the
  reading and `scripts/ported-from.test.ts` holds it honest: every address a comment names
  must lie inside a routine of its game, every formal citation must name the routine really
  at that address, and the committed pages must match what the walk writes now, so a change
  to the port without a rerun fails the suite. An address in a hole in Ghidra's function
  table goes in `KNOWN_TABLE_GAPS` with its reason, and the test fails if the entry ever
  stops being needed.
