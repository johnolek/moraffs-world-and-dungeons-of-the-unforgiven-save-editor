...
| `gain_or_drain` | 2000:8189 | `game/port/combat.ts` | `game/endless/monsters.ts`, `game/port/state.ts` |
...
| `roll_char` | 3000:4c77 | `game/port/character.ts`, `game/port/hints.ts` | `app-state.svelte.ts`, `game/dotu-pic.js`, `game/port/state.ts`, `roller/NewCharacter.svelte`, `roller/session.ts` |
...
| `FUN_3000_9026` | 3000:9026 |  | `game/port/state.ts`, `play/Screen.svelte`, `play/boss-office.ts`, `play/engine.ts`, `play/help.ts`, `play/manual.ts`, `play/office.ts`, `play/section-screen.ts`, `play/tablet.ts` |
...
| `kill_monster` | 3000:b12d | `game/port/kills.ts` | `formulas/formulas.ts`, `game/endless/shadows.ts`, `game/port/drops.ts`, `game/port/rules.ts`, `game/port/screens.ts`, `game/port/state.ts`, `play/boxes.ts`, `play/drop-odds.ts` |
